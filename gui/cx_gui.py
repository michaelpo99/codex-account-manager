...
class CxGui:
    ...
    def _build_ui(self) -> None:
        ...
        ribbon_shell = ttk.Frame(self.root, padding=(8, 6, 8, 2), style="Ribbon.TFrame")
        ...
        ribbon.bind("<Configure>", lambda event: self.on_ribbon_configure(ribbon_canvas, ribbon_window))
        ribbon_canvas.bind("<Configure>", lambda event: self.on_ribbon_canvas_configure(ribbon_canvas, ribbon_window))
        ...
        target_group, target_buttons = self.create_ribbon_group(ribbon, "Target")
        target_group.pack(side="left", fill="y")
        ttk.Label(target_buttons, text="Target").pack(anchor="w", padx=4)
        target = ttk.Combobox(target_buttons, textvariable=self.target_var, values=["WSL", "Windows Native"], state="readonly", width=18)
        target.pack(anchor="w", padx=4, pady=(3, 2))
        ...
        self.add_busy_button(account_buttons, text="Refresh", icon="refresh", command=self.refresh_accounts).pack(side="left", padx=1)
        self.add_busy_button(account_buttons, text="Status", icon="status", command=self.refresh_status_all).pack(side="left", padx=1)
        self.add_busy_button(account_buttons, text="Add\nAccount", icon="add", command=self.add_account).pack(side="left", padx=1)
        self.add_busy_button(account_buttons, text="Save\nCurrent", icon="save", command=self.save_current).pack(side="left", padx=1)
        self.add_ribbon_separator(ribbon)

        selection_group, selection_buttons = self.create_ribbon_group(ribbon, "Selection")
        selection_group.pack(side="left", fill="y")
        self.add_busy_button(selection_buttons, text="Use\nSelected", icon="use", command=self.use_selected).pack(side="left", padx=1)
        self.add_busy_button(selection_buttons, text="Best", icon="best", command=self.switch_to_best).pack(side="left", padx=1)
        self.add_busy_button(selection_buttons, text="Status\nSelected", icon="status", command=self.refresh_status_selected).pack(side="left", padx=1)
        self.add_busy_button(selection_buttons, text="Remove", icon="remove", command=self.remove_selected).pack(side="left", padx=1)
        self.add_ribbon_separator(ribbon)

        scope_group, scope_buttons = self.create_ribbon_group(ribbon, "Scope")
        scope_group.pack(side="left", fill="y")
        self.add_busy_button(scope_buttons, text="Work", icon="scope_work", command=lambda: self.set_selected_scope("work")).pack(side="left", padx=1)
        self.add_busy_button(scope_buttons, text="Personal", icon="scope_personal", command=lambda: self.set_selected_scope("personal")).pack(side="left", padx=1)
        self.add_ribbon_separator(ribbon)

        backup_group, backup_buttons = self.create_ribbon_group(ribbon, "Backup")
        backup_group.pack(side="left", fill="y")
        self.add_busy_button(backup_buttons, text="Export\nAll", icon="export_all", command=self.export_all).pack(side="left", padx=1)
        self.add_busy_button(backup_buttons, text="Export\nSelected", icon="export_selected", command=self.export_selected).pack(side="left", padx=1)
        self.add_busy_button(backup_buttons, text="Export\nFiltered", icon="export_filtered", command=self.export_filtered).pack(side="left", padx=1)
        self.add_busy_button(backup_buttons, text="Import", icon="import", command=self.import_backup).pack(side="left", padx=1)
        self.add_busy_button(backup_buttons, text="Inspect", icon="inspect", command=self.inspect_backup).pack(side="left", padx=1)
        ...
        self.root.after_idle(lambda: main_pane.sash_place(0, 0, 430))
    ...
    def configure_styles(self) -> None:
        ...
        style.configure("Ribbon.TButton", padding=(4, 3))

    def create_ribbon_group(self, parent: ttk.Frame, title: str) -> tuple[ttk.Frame, ttk.Frame]:
        group = ttk.Frame(parent, padding=(4, 0), style="RibbonGroup.TFrame")
