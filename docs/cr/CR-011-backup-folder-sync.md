...
WSL target
-> 先轉成 WSL 可讀 path，例如 /mnt/g/My Drive/codex-backups
-> 若目錄在 WSL 內不可直接讀取，但 Windows 端可讀
-> 允許 GUI 先在 Windows 掃描並複製備份檔到 WSL 臨時目錄，再由 WSL `cx sync-import` 匯入
```
...
5. 同一份 settings 可在 Windows Native 與 WSL 共用 directory；若 WSL target 無法直接讀取該路徑，GUI 可自動使用 Windows-to-WSL staging bridge。
...
Google Drive、rclone 或其他同步工具可能產生 conflict copy、延遲同步，或只在 Windows 提供虛擬磁碟而無法直接從 WSL 存取。Sync 必須以 manifest metadata 與 authHash 判斷，不得只依賴檔案修改時間。
...
7. Windows path 在 WSL target 下會先轉成 WSL 可讀 path；若 WSL 無法直接存取但 Windows 可讀，GUI 會自動改用 Windows staging bridge。
8. Sync 匯入後 GUI 列表更新。
...
15. 同步目錄不存在、無權限、Windows staging 失敗或其他 sync 錯誤時，GUI 保持安靜，只寫 Activity log 與狀態列。
